      ...
      <div class="modal-body">
        <div class="row">
            ...
            <div class="col-md-12 mt-2">
                ...
                <input type="text" name="url_youtube" class="form-control" maxlength="255" placeholder="Url Video" required>
            </div>
        </div>
      </div>
...
@section('js')
    <script>
        ...
        $('#btn_modal').on('click',function(){
            ...
            $('#form_data').attr('action',`{{ route('panel.videos.postData') }}`);
        });
        ...
        function editData(self){
            ...
            $('#form_data').attr('action',`{{ route('panel.videos.editData',-1) }}`.replace(-1,self.attr('data-id')));
            $('[name="title"]').val(self.attr('data-title'));
            $('[name="url_youtube"]').val(self.attr('data-url_youtube'));
        }
        ...
        function getData(page=1){
            ...
            table_data.html(`
                <tr>
                <td style="width: 10px" class="text-center" colspan="4"><i class="fas fa-spin fa-spinner"></i> Loading</td>
                </tr>
            `);
            $.ajax({
                'url' : `{{ route('panel.videos.data') }}`,
                'type' : 'GET',
                'data' : {
                ...
                'success' : function(result){
                    if(result.data.length == 0){
                        table_data.html(`
                        <tr>
                            <td style="width: 10px" class="text-center" colspan="4">Data Tidak Ditemukan</td>
                        </tr>
                        `);
                    }else{
                        table_data.html('');
                            ...
                            table_data.append(`
                                <tr>
                                    ...
                                    <td style="width: 10px" class="text-center">
                                        ...
                                        data-url_youtube="${val['url_youtube']}"></i>
                                    </td>
                                    <td>${val['title']}</td>
                                    <td>
                                        <a href="${val['url_youtube']}" target="_blank">${val['url_youtube']}</a>
                                    </td>
                                </tr>
                            `);
                        });
                    ...
                    table_data.html(`
                        <tr>
                            <td style="width: 10px" class="text-center" colspan="4">Terjadi Kesalahan Sistem...</td>
                        </tr>
                    `);
                }
            });
        ...
        function deleteData(self){
            ...
            if(konfirm == true){
                ...
                $.ajax({
                    'url' : `{{ route('panel.videos.deleteData',-1) }}`.replace(-1,self.attr('data-id')),
                    'type' : 'delete',
                    'data' : {
