...
class SlidersController extends Controller
{
    public function index(Request $req){
        ...
        $object_data = [];
        $object_data['page_title'] = "Data Sliders";
        $object_data['datawebsite'] = $datawebsite;
        return view('panel.pages.sliders.index',$object_data);
    }
        ...
        try {
            ...
            if($req->hasFile('images')) {
                if (str_contains($required_ext, @$req->images->extension())) { 
                    ...
                    $req->images->move(public_path('upload/sliders'), $name_images);
                    $save_images = '/upload/sliders/'.$name_images;
    ...
    public function editData(Request $req, $id){
        ...
        Slider::where('id',$id)->update($object_data);
        return response()->json([
            'status' => 200, 'messages' => 'Save Successfull', 'save_images' => @$save_images
        ]);
